...
import { GET_USER_INFO, HOST, REMOVE_PROFILE_IMAGE_ROUTE, UPDATE_PROFILE_ROUTE } from '@/utils/constants';
  ...
  const handleImageChange = async(event)=>{
    ...
    if(file){
      ...
      if (response.status === 200 && response.data.image) {
        ...
        const userInfoResponse = await apiClient.get(GET_USER_INFO, {
          withCredentials: true,
        });
        ...
        if (userInfoResponse.status === 200) {
          ...
          // ✅ Set full image URL (use HOST)
          if (updatedUser.image) {
            setimage(`${HOST}/${updatedUser.image}`);
          }
        }
      }
    }
  ...
  <div className="bg-[#1b1c24] h-[100vh] flex items-center justify-center flex-col gap-10">
    <div className="flex flex-col gap-10 w-[80vw] md:w-max">
      ...
      <div className="grid grid-cols-2">
        ...
        <div className="flex min-w-32 md:minw64 flex-col gap-5 text-white items-center justify-center">
          ...
          <div className='w-full'>
            ...
            <div className="w-full flex gap-5 pt-3">
              {colors.map((color, index) => (
                <div
                className={`${getColor(index)} h-8 w-8 rounded-full cursor-pointer transition-all duration-300 
